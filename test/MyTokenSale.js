...
contract('MyTokenSale', function (accounts) {
    ...
    it('initializes the contract with the correct values', function () {
        return MyTokenSale.deployed().then(function (instance) {
            ...
            assert.notEqual(address, 0x0, 'has token correct address');
    ...
    it('end token sale', function () {
        return MyToken.deployed().then(function (instance) {
            ...
            assert(error.message, 'must be admin to end the sale');
            return tokenSaleInstance.endSale({ from: admin });
        }).then(function (receipt) {
            return tokenInstance.balanceOf(admin);
        }).then(function (balance) {
            assert.equal(balance.toNumber(), 999990, 'return all tokens to admin');

            const getBalance = async () => {
                balance = await web3.eth.getBalance(address);
                assert.equal(balance.toNumber(), 0, "return");
